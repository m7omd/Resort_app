...
class StatsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    const Color orangeColor = Color(0xFFE28731); // لون تم الدفع جزئياً من الصورة

    return Scaffold(
      backgroundColor: const Color(0xFFEBF1F5), // نفس الخلفية الرمادية الفاتحة
      appBar: AppBar(
        title: const Text(
          'إحصائيات الشهر',
        ...
        builder: (context, state) {
          if (state is HomeLoading) {
            ...
            for (var booking in bookings) {
              ...
              if (booking.isPaid) {
                totalReceivedIncome += booking.totalPrice;
                paidInFullCount++;
              } else {
                totalRemainingIncome += booking.totalPrice;
                partialPaidCount++;
              }
            }
            ...
            return Center(child: Text('حدث خطأ أثناء تحميل البيانات: ${state.error}'));
          }
